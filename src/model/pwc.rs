...
impl PieceWiseConstantFunction {
    pub fn from_points(min: f64, max: f64, buckets: usize, min_bucket_population: usize, points: &[(f64, f64)]) -> PieceWiseConstantFunction {
        PieceWiseConstantFunction::new(min, max, buckets, min_bucket_population, |s, e| {
            points.iter().cloned().filter_map(move |(x, y)| {
                if x >= s && x < e {
                    Some(y)
                } else {
                    None
    ...
    pub fn new<F, I>(min: f64, max: f64, buckets: usize, min_bucket_population: usize, f: F) -> PieceWiseConstantFunction
        where F: Fn(f64, f64) -> I,
              I: Iterator<Item=f64> {
        let mut pwc = Vec::new();
...
mod tests {
    ...
    fn values_not_in_domain_are_skipped() {
        let h = PieceWiseConstantFunction::from_points(0.0, 1.0, 1, 1,
                                                       &[(-1.0, 12.0), (2.0, 23.0)]);
